...
5. **Given** a student reading the cleaning section, **When** they finish, **Then** they have seen and applied: dtype coercion via `pd.to_numeric(errors="coerce")` on an inline synthetic Series modelling the "Less than 1 year" sentinels commonly encountered in historical / other CSVs, handling of `NaN` (`isna`, `fillna`, `dropna`), and idempotent application of the numeric-coercion pattern to the already-clean 2025 `YearsCode` / `WorkExp` columns. The section is honest that 2025 Survey is pre-cleaned; the synthetic example carries the transferable technique. Datetime parsing is explicitly out of scope for this lecture.
...
- **Part 1 — in-class (~10–15 min)**: A simple filter + aggregate question, e.g., "What is the median `WorkExp` among Ukrainian respondents, and how does it compare to the global median?" (`WorkExp` replaces the pre-2025 `YearsCodePro` column; the 2025 schema dropped `YearsCodePro`.)
...
- **FR-010**: The notebook MUST include a cleaning section covering: dtype coercion (`astype`, `pd.to_numeric(errors="coerce")`), missing-value handling (`isna`, `fillna`, `dropna` — with the difference between "drop rows with any NaN" vs "drop rows where a specific column is NaN"), and at least one example of coercing a messy numeric-ish column with string sentinels like "Less than 1 year" and "More than 50 years" — demonstrated on a small inline synthetic `pd.Series` because the 2025 Survey is pre-cleaned (`YearsCode` / `WorkExp` ship as `float64`). The coercion pattern is then applied idempotently to the real columns so students see it's safe to run on already-clean data. Datetime parsing is explicitly out of scope — the Survey CSV ships no submission-timestamp column, and the lecture is constrained to a single dataset.
...
- **Stack Overflow Developer Survey record** (the only entity in this lecture): One respondent's answers to the survey; a single row in the CSV. Key columns the notebook works with (2025 Survey schema) include: `ResponseId`, `MainBranch`, `Country`, `EdLevel`, `YearsCode` (total years coding, already numeric in 2025), `WorkExp` (years of professional work experience — **replaces** pre-2025 `YearsCodePro` which was dropped in the 2025 schema), `DevType`, `LanguageHaveWorkedWith`, `LanguageWantToWorkWith`, `DatabaseHaveWorkedWith`, `ConvertedCompYearly`, `RemoteWork`. Not persisted — only loaded into pandas DataFrames inside the notebook.
